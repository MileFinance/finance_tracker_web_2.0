      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-4">
        ...
        <section className="flex flex-wrap gap-2">
          ...
          <Link
            ...
            className="rounded-xl border border-[#252545] bg-[#04040a] px-4 py-2 text-sm font-semibold text-white"
          >
            Portfolio Settings
          </Link>
        </section>
        ...
        <section className="grid gap-3 md:grid-cols-2">
          {loading ? (
            <p className="rounded-2xl border border-[#252545] bg-[#07070e] p-4 text-sm text-neutral-300">Loading portfolios...</p>
          ) : null}
          {error ? (
            <p className="rounded-2xl border border-rose-500/20 bg-rose-500/10 p-4 text-sm text-rose-300">{error}</p>
          ) : null}
          {!loading && !error && portfolios.length === 0 ? (
            <p className="rounded-2xl border border-[#252545] bg-[#07070e] p-4 text-sm text-neutral-300">No portfolios available yet.</p>
          ) : null}

          {portfolios.map((portfolio) => {
            const active = portfolio.id === selectedPortfolioId;
            return (
              <article key={portfolio.id} className="rounded-2xl border border-[#252545] bg-[#07070e] p-5">
                  ...
                  <button
                    ...
                    className={`rounded-xl border px-4 py-2 text-sm font-semibold ${
                      active
                        ...
                        : "border-[#252545] bg-[#04040a] text-white"
                    }`}
                  >
